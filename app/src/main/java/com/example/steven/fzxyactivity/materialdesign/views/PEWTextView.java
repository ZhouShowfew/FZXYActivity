...
import com.example.steven.fzxyactivity.materialdesign.utils.AttrConstants;
import com.example.steven.fzxyactivity.materialdesign.utils.InterpolatorSelector;
...
public class PEWTextView extends TextView {
    ...
    private void applyParallax() {
        ...
        if (scrollSpaceY != 0
            ...
            if (reverseY)
                setMyScrollY((int) (
                        Math.min(Math.max((0.5f - interpolatedScrollDeltaY), -0.5f), 0.5f) * -scrollSpaceY));
            else
                setMyScrollY((int) (
                        Math.min(Math.max((0.5f - interpolatedScrollDeltaY), -0.5f), 0.5f) * scrollSpaceY));
        }
        ...
        if (scrollSpaceX != 0
            ...
            if (reverseX) {
                setMyScrollX((int) (
                        Math.min(Math.max((0.5f - interpolatedScrollDeltaX), -0.5f), 0.5f) * -scrollSpaceX));
            } else {
                setMyScrollX((int) (
                        Math.min(Math.max((0.5f - interpolatedScrollDeltaX), -0.5f), 0.5f) * scrollSpaceX));
            }
